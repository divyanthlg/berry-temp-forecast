...
def build_gru(n_in: int, n_features: int, n_out: int, cfg: dict) -> Model:
    units_1, units_2 = cfg["layer_units"]
    dense_units = cfg["dense_units"]
    dropout_rate = cfg["dropout"]

    inp = Input(shape=(n_in, n_features), name="input_seq")
    x = GRU(units_1, return_sequences=True, name="gru_1")(inp)
    x = Dropout(dropout_rate, name="dropout_1")(x)
    x = GRU(units_2, return_sequences=False, name="gru_2")(x)
